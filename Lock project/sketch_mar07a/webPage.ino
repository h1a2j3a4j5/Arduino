...
if(server.hasArg("one")) {
   number = server.arg("send"); 
   int allCode = number.toInt();

   for(int x = number.length()-1; x>=0 ;x--){
       correctCode[x]= allCode % 10;
       allCode /= 10;       
   }
...
strcat(html,"    <script src=\"./script.js\"></script>\n");
strcat(html,"    </head>\n");
strcat(html,"    <body>\n");
strcat(html,"    <form action=\"\" method=\"get\">\n");
strcat(html,"    <P>\n");
strcat(html,"    <input id=\"send\" type=\"text\" value=\"\" name=\"send\">\n");
strcat(html,"    </P>\n");
strcat(html,"    <button type=\"submit\" name=\"one\">press</button>\n");
strcat(html,"    </form>\n");
strcat(html,"    </body>\n");
strcat(html,"    </html>");
